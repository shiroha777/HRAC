...
\begin{itemize}
    \item \textbf{Robust aggregation implementation:} a modular server-side aggregation component (PyTorch-based) implementing the proposed \emph{HSM-Adaptive} method and baseline aggregators within a federated learning pipeline.
...
The proposed method, \emph{HSM-Adaptive}, computes a soft trust score for each client update by combining complementary signals:
\begin{itemize}
    \item \textbf{Population alignment:} cosine alignment of $\Delta_{t,i}$ with a server-side reference direction (previous aggregate or momentum), robustly centred using the median to reduce sensitivity to non-IID shifts.
    \item \textbf{Self-consistency:} agreement between a client's current update structure and its own historical behaviour, estimated using a low-dimensional sign-sketch representation to tolerate noise while remaining sensitive to abrupt behavioural change.
    \item \textbf{Scale deviation:} a robust log-ratio measure comparing the current update norm to the client's historical norm profile to detect abnormal scaling.
\end{itemize}
These signals are combined into a continuous trust score and transformed into multiplicative aggregation weights using smooth gating functions, avoiding hard exclusion and improving stability under benign heterogeneity.

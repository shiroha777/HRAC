"""
绘制 Non-IID 环境下 label flipping 攻击的 HSM vs Mean 对比图
"""
...
dataset = 'cifar10'
partition_name = 'LabelSeperation'  # Non-IID 环境（对应 noniid）
...
def draw_noniid_label_flipping():
    """绘制 Non-IID 环境下 label flipping 攻击的训练曲线"""
    ...
    for agg_config in aggregations_to_load:
        ...
        if not acc_path or len(acc_path) == 0:
            print(f"警告: {agg_config['file_name']} 没有有效数据")
            continue
    ...
    title = f'CIFAR-10 - Non-IID Partition - Label Flipping Attack (b=3)'
    ...
    pic_name = f'{method}_{dataset}_label_flipping_noniid_b3_comparison'
